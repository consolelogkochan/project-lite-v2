...
>
    ...
    <div 
        ...
        @mousedown.stop
        class="relative z-10 w-full max-w-5xl overflow-y-auto bg-gray-100 dark:bg-gray-800 rounded-lg shadow-xl"
    ...
    >
        ...
        <div x-show="selectedCardData" x-cloak>
            ...
            <div class="p-6 grid grid-cols-9 gap-6">

                {{-- ★★★ メインコンテンツ (左側 / 2カラム分) ★★★ --}}
                <div class="col-span-9 lg:col-span-5 space-y-6">

                    {{-- ★★★ カードタイトルセクション ★★★ --}}
                    <div class="flex items-start space-x-3">
                        {{-- ★ 1. [NEW] 完了チェックボックス (SVGアイコンの位置に移動) --}}
                        <div class="flex-shrink-0 pt-1">
                            <input type="checkbox" 
                                {{-- 1. 状態をバインド (null ガード付き) --}}
                                :checked="selectedCardData && selectedCardData.is_completed"
                                {{-- 2. 変更イベントを発火 --}}
                                @change="$dispatch('toggle-card-completed', { 
                                    card: selectedCardData,
                                    isCompleted: $event.target.checked 
                                })"
                                class="rounded border-gray-300 dark:border-gray-600 text-blue-600 shadow-sm focus:ring-blue-500 w-7 h-7">
                        </div>
                        {{-- ★ 1. [NEW] チェックボックスここまで --}}
                    ...
                    {{-- ★★★ 割り当て済みメンバー一覧 ★★★ --}}
                    <div x-show="selectedCardData && selectedCardData.assignedUsers && selectedCardData.assignedUsers.length > 0" x-cloak>
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Members</h3>
                        <div class="flex flex-wrap gap-2 mt-2">
                            {{-- ループ --}}
                            <template x-for="member in (selectedCardData ? selectedCardData.assignedUsers : [])" :key="member.id">
                                <div :title="member.name">
                                    {{-- アバターURL (アクセサ) が存在する場合 --}}
                                    <template x-if="member.avatar_url">
                                        <img class="h-8 w-8 rounded-full object-cover ring-2 ring-violet-500" :src="member.avatar_url" :alt="member.name">
                                    </template>
                                    {{-- フォールバック: イニシャル --}}
                                    <template x-if="!member.avatar_url">
                                        <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gray-500 dark:bg-gray-600 ring-2 ring-violet-500">
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            ...
                            >
                                ...
                                <button @click="open = !open" 
                                        type="button" 
                                        class="w-full flex items-center bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 text-xs font-medium rounded-md px-3 py-2">
                                ...
                                >
                                    <div class="p-4">
                                        <h4 class="text-center text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Members</h4>
                                        ...
                                        <p class="mt-4 mb-2 text-xs text-gray-500 dark:text-gray-400">Board members</p>
                                        
                                        {{-- メンバー一覧ループ --}}
                                        <div class="space-y-1 max-h-48 overflow-y-auto">
                                            {{-- $parent.boardMembers で親スコープのデータを参照 --}}
                                            <template x-for="member in boardMembers.filter(m => m.name.toLowerCase().includes(search.toLowerCase()) || m.email.toLowerCase().includes(search.toLowerCase()))" :key="member.id">
                                                <div class="flex items-center space-x-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
                                                    {{-- ★★★ 修正後のチェックボックス ★★★ --}}
                                                    <input type="checkbox"
                                                        {{-- 1. 割り当て済みかチェック (null ガード付き) --}}
                                                        :checked="selectedCardData && selectedCardData.assignedUsers && selectedCardData.assignedUsers.some(u => u.id === member.id)"
                                                        {{-- 2. クリックでイベント発火 --}}
                                                        @change="$dispatch('toggle-assigned-user', { 
                                                            card: selectedCardData, 
                                                            member: member,
                                                    ...
                                                    {{-- 名前 --}}
                                                    <span class="text-sm font-medium text-gray-900 dark:text-gray-100" x-text="member.name"></span>
                                                </div>
                                            </template>
                                        </div>
                            ...
                            <div x-data='{ open: false }' 
                                ...
                                <button @click="open = !open" 
                                        type="button" 
                                        class="w-full flex items-center bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 text-xs font-medium rounded-md px-3 py-2">
                                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h.01M17 3h.01M17 7h.01M17 11h.01M17 15h.01M7 11h.01M7 15h.01M3 21h18M3 17h18M3 13h18M3 9h18M3 5h18"></path></svg>
                                    Labels
                                </button>
                            ...
                            >
                                ...
                                <button @click="open = !open; if(open) { $nextTick(() => initPickers()) }" 
                                        type="button" 
                                        class="w-full flex items-center bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 text-xs font-medium rounded-md px-3 py-2">
                                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 002-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    Dates
                                </button>
                            ...
                            >
                                ...
                                <label for="file-upload"
                                    class="w-full flex items-center bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 text-xs font-medium rounded-md px-3 py-2 cursor-pointer">
                                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.414a4 4 0 00-5.656-5.656l-6.415 6.415a6 6 0 108.486 8.486L20.5 13"></path></svg>
                                    Attachment
                                </label>
